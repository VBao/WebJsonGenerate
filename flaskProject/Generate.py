...
class Generate:
    ...
    class people:
        def name():
            ...
            f=open('../Dictionary/firstname.txt','r+').readlines()
            return str(random.choice(l).strip('\n')+' '+ random.choice(f).strip('\n'))
        def gender(*arsg):
            if len(arsg) ==0:
        ...
        def add():
            l=open('../Dictionary/lastname.txt','r+').readlines()
            f=open('../Dictionary/firstname.txt','r+').readlines()
            return str(random.randint(1,500))+ ' '+ str(random.choice(l).strip('\n')+' '+ random.choice(f).strip('\n'))